...
### AE-3. 첫 정기 발송(9/28) 전 최종 점검 (2026-09-23)
- **Gmail 잘림 방지**: Gmail 은 HTML 이 약 102KB 를 넘으면 뒷부분을 숨겨 면책 고지·구독해지 링크가 가려진다. 주요 항목 1건 ≈ 2.4KB 이므로 `MAIN_FULL_MAX = 25` 건까지만 발췌 포함, 나머지는 "그 밖의 주요 항목"에 영향도·날짜·기관·제목 한 줄로. 주요 60 + 참고 60 건에서도 100KB 미만임을 테스트로 고정.
...
- 메일의 "정보 수집 기간" 시작을 실제 실린 항목 중 가장 이른 수집 시각으로 표기(첫 리포트 14일치와 일치). 구독자의 첫 리포트에는 "최근 14일 수집분을 함께 담았다" 안내 한 줄. 테스트 발송은 정기 발송 기록이 없을 때(첫 발송 전) 이 안내까지 미리 보여준다. "매일 08:00 수집" → "매일 오전 8시경 수집".
- 영향도 라벨 색 구분: 즉시 조치=빨강 채움, 검토 필요=노랑 바탕+갈색 글자+테두리, 참고=회색 테두리. 머리글에 "RegTide 수집 기간"과 "기관 발표일" 범위를 따로 표기하고, 발표일이 수집일보다 앞설 수 있다는 설명 추가.
- 테스트 수신자: 대시보드 "운영 작업" 탭에서 추가·삭제(최대 5개, page_snapshots 의 admin:test_emails 행에 저장 — 마이그레이션 없음). "테스트 다이제스트 발송" 버튼이 운영자 + 등록 주소에 각 1통씩 보낸다(제목 [테스트], 발송 기록 없음). Vercel 환경변수 TEST_EMAILS 도 계속 인정(대시보드에서는 삭제 불가로 표시). 운영 리포트 수신자(ADMIN_EMAIL)는 여전히 운영자 도메인만 허용.
